...
public enum Sprite {
  ...
  private static EnumMap<Sprite, BufferedImage> cached_full_images =
      new EnumMap<Sprite, BufferedImage>(Sprite.class);
  private static EnumMap<Sprite, BufferedImage[][]> cached_sprites =
      new EnumMap<Sprite, BufferedImage[][]>(Sprite.class);
  ...
  /**
   * Treats the sprite as a two dimensional array of images. This is useful for connectable sprites
   * or sprites with different variants.
   *
   * <p>Some sprites may have multiple rows where a random row should be drawn every time the sprite
   * is rendered.
   *
   * <p>Additionally, some sprites may have multiple columns where the correct column number depends
   * on the surrounding blocks. The typical column scheme is as follows:
   *
   * <ul>
   *   <li>0: The block is surrounded by air/not connected to anything
   *   <li>1: The block is connected from the bottom
   *   <li>2: The block is connected from the bottom and the right
   *   <li>3: The block is connected from the bottom and the top
   *   <li>4: The block is connected from the bottom, left, and right
   *   <li>5: The block is connected from all sides
   * </ul>
   */
